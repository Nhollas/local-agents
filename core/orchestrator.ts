...
import type { TrackerAdapter, CodeHostAdapter, Config, RepoWorkflow, Issue } from "./types.ts";
    ...
    try {
      ...
      const results = await Promise.allSettled(
        entries.map(async ([repo, workflow]) => {
          ...
          return issues.map((issue): TaggedIssue => ({ issue, repo, workflow }));
        }),
      );
      ...
      for (const [i, result] of results.entries()) {
        if (result.status === "fulfilled") {
          ...
          logger.warn({ repo: entries[i][0], err: result.reason }, "orchestrator.fetch_failed");
        }
      }

      // 2. SORT by createdAt ascending (oldest first)
      allTagged.sort((a, b) => a.issue.createdAt.localeCompare(b.issue.createdAt));
      ...
      for (const { issue, repo, workflow } of allTagged) {
        ...
        const ws = await ensureWorkspace(issue, defaults.workspace_root, cloneUrl, workflow.hooks);
