 ...
 *
 * Clones the repo, reads the PR diff, explores the codebase for established
 * patterns, and returns a structured list of convention violations.
 *
 * Called by the webhook server when a /check command is received.
 */
...
/** Run the conventions check for a PR. Called by the server on /check. */
export async function runCheck(
  repo: string,
  prNumber: number,
  headBranch: string,
  baseBranch: string,
  config: { model: string; maxFindingsPerCheck: number; workDir: string; dataDir: string }
): Promise<void> {
  ...
  try {
    ...
    await cloneAndCheckout(repo, headBranch, workDir);
